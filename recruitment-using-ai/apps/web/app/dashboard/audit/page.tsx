...
const TYPE_CONFIG: Record<string, { color: string; icon: string; label: string }> = {
  RESUME_SCORE: { color: '#10b981', icon: 'ðŸ“‹', label: 'Resume Scoring' },
  VIDEO_EVAL:   { color: '#667eea', icon: 'ðŸŽ¥', label: 'Video Evaluation' },
  SHORTLIST:    { color: '#a78bfa', icon: 'âš–ï¸', label: 'Shortlisted' },
  OFFER:        { color: '#f59e0b', icon: 'ðŸ“', label: 'Offer Generated' },
  BIAS_CHECK:   { color: '#f97316', icon: 'ðŸ”', label: 'Bias Audit' },
  FEEDBACK:     { color: '#ec4899', icon: 'ðŸ’¬', label: 'Manager Feedback' },
}
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f0c29, #1a1a2e, #16213e)', fontFamily: 'Segoe UI, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 32 }}>
          <div style={{ width: 52, height: 52, background: 'linear-gradient(135deg, #667eea, #764ba2)', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>ðŸ—‚ï¸</div>
          <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 28 }}>
          {[
            { label: 'Total Decisions', value: stats.total,     icon: 'ðŸ—‚ï¸', color: '#667eea' },
            { label: 'Resume Scores',   value: stats.resume,    icon: 'ðŸ“‹', color: '#10b981' },
            { label: 'Video Evals',     value: stats.video,     icon: 'ðŸŽ¥', color: '#a78bfa' },
            { label: 'Shortlisted',     value: stats.shortlist, icon: 'âš–ï¸', color: '#f59e0b' },
          ].map(s => (
            ...
            return (
              <div key={log.id} style={{ display: 'grid', gridTemplateColumns: '160px 1fr 1fr 1fr 80px 120px', gap: 16, padding: '16px 24px', borderBottom: i < filtered.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none', alignItems: 'center' }}
                ...
                <p style={{ color: log.score !== undefined ? (log.score >= 70 ? '#10b981' : log.score >= 50 ? '#f59e0b' : '#ef4444') : 'rgba(255,255,255,0.3)', fontSize: 14, fontWeight: 700, margin: 0 }}>
                  {log.score !== undefined ? log.score : 'â€”'}
                </p>
