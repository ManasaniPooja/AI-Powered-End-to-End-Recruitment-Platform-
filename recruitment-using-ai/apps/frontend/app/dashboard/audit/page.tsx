...
const TYPE_CONFIG: Record<string, { color: string; icon: string; label: string }> = {
  ...
  OFFER:        { color: '#f59e0b', icon: '📄', label: 'Bias Audit' },
          ...
          {loading ? (
            ...
            return (
              <div key={log.id} style={{ display: 'grid', gridTemplateColumns: '160px 1fr 1fr 1fr 80px 120px', gap: 16, padding: '16px 24px', borderBottom: i < filtered.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none', alignItems: 'center' }}
                ...
                <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{log.candidate}</p>
                <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{log.job}</p>
